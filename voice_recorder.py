...
from config import VOICE_DIR, VOICE_SAMPLE_RATE, VOICE_CHANNELS, VOICE_FORMAT
...
def get_recent_recordings(limit: int = 10) -> list[VoiceRecording]:
    """
    Get list of recent voice recordings.
    
    Args:
        limit: Maximum number of recordings to return
        
    Returns:
        List of VoiceRecording objects, sorted by creation time (newest first)
    """
    recordings = []
    ...
    try:
        ...
        for filepath, filename in files[:limit]:
            try:
                # Parse recording info from filename
                # Format: voice_YYYYMMDD_HHMMSS_ID.wav
                parts = filename.replace(f'.{VOICE_FORMAT}', '').split('_')
                if len(parts) >= 4:
                    date_part = parts[1]
                    time_part = parts[2]
                    ...
                    created_at = dt.datetime.strptime(datetime_str, "%Y%m%d_%H%M%S")
                    ...
                    recording = VoiceRecording(
                        ...
                        duration=None,  # Would need to parse WAV header for exact duration
                        transcription=None,
                        processed=False
                    )
